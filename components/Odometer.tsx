...
function OdometerDigit({ targetValue }: { targetValue: number }) {
  return (
    <span className="relative inline-block overflow-hidden h-[1.2em] leading-[1.2em] min-w-[0.55em] text-center align-bottom">
      <span
        className="absolute top-0 left-0 flex flex-col transition-transform duration-[1500ms] ease-[cubic-bezier(0.2,1,0.2,1)]"
        style={{ transform: `translateY(-${targetValue * 1.2}em)` }}
      >
        {NUMBERS.map((num) => (
...
export default function Odometer({ value }: { value: number }) {
  ...
  return (
    <span className="inline-flex font-medium items-baseline">
      {chars.map((char, index) => {
        ...
        return <span key={key} className="inline-block -mx-[0.1em]">{char}</span>;
      })}
    </span>
  );
}
